...
def run_stream_miplosc(
    ...
    MCS_upd = copy.deepcopy(micro_clustering_system)
    MCS_truth.set_seed(random_state)
    MCS_upd.set_seed(random_state)

    classes = np.unique(Y)

    if verbose >= 1:
        print(f"\n========== [Mode] <MIPLOSC>-delay_{np.mean(delay)} ==========")
    ...
    if n_pretrain is not None and n_pretrain > 0:
        ...
        MCS_upd.initialize_MCs(initial_X, initial_Y, k)

        MCS_truth.set_time(stream.current_time_step())
        MCS_upd.set_time(stream.current_time_step())
        if verbose >= 2:
    ...
    while stream.has_next():
        ...
        ''' Training '''
        sampling_MCs = []
        sampling_removed_MCs = []
        conflict_happened = False
        if train_x is not None and train_y is not None:
            for idx, x, y in zip(train_index, train_x, train_y):
                truth_conflict, truth_MCs, truth_removed_MCs = MCS_truth.partial_fit([x], [y],
                                                                                     is_pseudo=False)
                conflict, MCs, removed_MCs = MCS_upd.partial_fit([x], [y], is_pseudo=False)

                if conflict:
                    sampling_MCs.extend(MCs)
                    sampling_removed_MCs.extend(removed_MCs)
            ...
            if pseudo_x is not None and pseudo_y is not None:
                for px, py in zip(pseudo_x, pseudo_y):
                    conflict, MCs, removed_MCs = MCS_upd.partial_fit([px], [py], is_pseudo=True)
                    classifier.partial_fit([px], [py])

            if len(sampling_MCs) != 0 or conflict_happened:
                x_train_cls, y_train_cls = MCS_upd.sampling_by_KNN(sampling_MCs, k_in_knn, extra_sampling_num)
                if x_train_cls is not None:
                    classifier.partial_fit(x_train_cls, y_train_cls)
    ...
    if show_plot:
        plot_res(dataset, running_records)

    return running_records


def plot_res(dataset, running_records):
